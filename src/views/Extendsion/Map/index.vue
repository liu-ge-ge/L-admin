<template>
  <div class="searchBox">
    ...
    <div class="searchResult">
      <div v-if="keyword" class="item" v-for="item in poiList?.pois" :key="item.id" @click="setMapCenter(item)">
        <span>{{ item.name }}</span>
  ...
  <a-card class="l-card" title="高德地图"> </a-card>
  <transition name="map">
    <div class="select" v-if="isShowTheme">
      <a-radio-group v-model:value="themeValue" @change="changeMapTheme">
        <a-radio :style="radioStyle" value="normal" class="radio">标准</a-radio>
        <a-radio :style="radioStyle" value="macaron" class="radio">马卡龙</a-radio>
        <a-radio :style="radioStyle" value="graffiti" class="radio">涂鸦</a-radio>
        <a-radio :style="radioStyle" value="whitesmoke" class="radio">远山黛</a-radio>
        <a-radio :style="radioStyle" value="dark" class="radio">幻影黑</a-radio>
        <a-radio :style="radioStyle" value="fresh" class="radio">草青色</a-radio>
        <a-radio :style="radioStyle" value="darkblue" class="radio">极夜蓝</a-radio>
        <a-radio :style="radioStyle" value="blue" class="radio">靛青蓝</a-radio>
        <a-radio :style="radioStyle" value="light" class="radio">月光银</a-radio>
        <a-radio :style="radioStyle" value="grey" class="radio">雅士灰</a-radio>
      </a-radio-group>
    </div>
  </transition>
  ...
  <div
    ...
    class="control"
    :style="{ color: isShowTheme ? 'var(--ant-primary-color)' : 'var(--ant-text-color)' }"
  >
  ...
  <div
    @click="changeMapMode"
    class="control3D"
    :style="{ color: isShow3D ? 'var(--ant-primary-color)' : 'var(--ant-text-color)' }"
  >
...
const initMap = () => {
  AMapLoader.load({
    ...
    version: '2.0',
    plugins: ['AMap.ToolBar', 'AMap.Driving', 'AMap.ControlBar', 'AMap.PlaceSearch', 'AMap.AutoComplete'],
    Loca: {
      version: '2.0.0',
    },
  })
